...
function removeFirstOccurrences(str, value) {
  ...
  const strEnd = str.slice(str.indexOf(value) + value.length, str.length);
  return str.includes(value) ? strStart + strEnd : str;
}
...
function removeLastOccurrences(str, value) {
  const strStart = str.slice(0, str.lastIndexOf(value));
  const strEnd = str.slice(str.lastIndexOf(value) + value.length, str.length);
  return str.includes(value) ? strStart + strEnd : str;
}
...
function formatTime(minutes, seconds) {
  ...
  if (minutes.toString().length < 2) {
    minut = minutes.toString().padStart(2, '0');
  } else {
    minut = minutes.toString();
  }
  if (seconds.toString().length < 2) {
    sec = seconds.toString().padStart(3, ':0');
  } else {
    sec = seconds.toString().padStart(3, ':');
  }
  return minut + sec;
}
...
function countVowels(str) {
  ...
  let sum = 0;
  for (let i = 0; i <= str.length; i += 1) {
    if (vowels.includes(str[i])) sum += 1;
  }
  return sum;
}
...
function isPalindrome(str) {
  let strCopy = str.toLowerCase().replaceAll(' ', '').replaceAll(',', '');
  strCopy = strCopy.replaceAll('!', '').replaceAll('?', '');
  return strCopy === strCopy.split('').reverse().join('');
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str[i] !== ' ' && str[i] !== '!' && str[i] !== '?') {
      str2 += output[input.indexOf(str[i])];
    } else str2 += str[i];
  }
  return str2;
}
